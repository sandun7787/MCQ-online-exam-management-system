...
$userdata = "SELECT * FROM mcqsystem.user WHERE roleid='" . $_SESSION['roleid'] . "'";
...
$sql1 = "SELECT exams.id AS id, exams.status As status, exams.name AS name, exams.dateandtime AS dateandtime, exams.duration AS duration, examenrollment.Examstatus AS Examstatus FROM mcqsystem.exams LEFT JOIN mcqsystem.examenrollment ON mcqsystem.exams.id = mcqsystem.examenrollment.Exam_id AND (mcqsystem.examenrollment.student_id ='$_SESSION[student_login_id]' OR mcqsystem.examenrollment.student_id IS NULL) WHERE mcqsystem.exams.status = 'End'or mcqsystem.exams.status='published' ORDER BY exams.dateandtime DESC";
